...
func _update_stick(pointer_position: Vector2) -> void:
	...
	direction_changed.emit(direction.normalized() if direction.length_squared() > 0.0 else Vector2.ZERO)
	queue_redraw()
...
func _stick_center() -> Vector2:
	...
	return Vector2(STICK_MARGIN + STICK_RADIUS, viewport_size.y - STICK_MARGIN - STICK_RADIUS)

func _run_rect() -> Rect2:
	var viewport_size := get_viewport_rect().size
	return Rect2(
		Vector2(viewport_size.x - STICK_MARGIN - RUN_SIZE.x, viewport_size.y - STICK_MARGIN - RUN_SIZE.y - 10.0),
		RUN_SIZE
	)
...
func _draw() -> void:
	...
	draw_rect(run_rectangle, Color(0.851, 0.925, 0.439, 0.86), true)
	draw_rect(run_rectangle, Color(0.945, 0.957, 0.867, 0.55), false, 2.0, true)
	draw_string(
